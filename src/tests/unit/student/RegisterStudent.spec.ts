...
describe("RegisterStudent Use Case", () => {
  ...
  it("should register a new student", async () => {
    ...
    expect(await bcrypt.compare(studentData.password, student.password)).toBe(true);
  ...
  it("should throw an error if student already exists", async () => {
    ...
    await expect(registerStudent.execute(studentData)).rejects.toThrow("Student already exists with this email");
  });
});
