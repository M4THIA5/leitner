...
export class LoginStudent {
  ...
  async execute(params: { email: string; password: string }): Promise<{ student: Student; token: string }> {
    const student = await this.studentRepository.findByEmail(params.email);
    if (!student) {
      throw new Error("Invalid credentials");
    }

    const isPasswordValid = await bcrypt.compare(params.password, student.password);
    if (!isPasswordValid) {
      throw new Error("Invalid credentials");
    }
