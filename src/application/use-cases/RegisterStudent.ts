...
export class RegisterStudent {
  ...
  async execute(params: { name: string; email: string; password: string }): Promise<Student> {
    const existingStudent = await this.studentRepository.findByEmail(params.email);
    if (existingStudent) {
